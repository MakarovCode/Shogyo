$(document).on('turbolinks:load', function() {
	...
	if($('#subscribe-modal').length > 0) {
		show_modal = "#subscribe-modal";
	}
	if ($('#logged-modal').length > 0){
		show_modal = "#logged-modal";
	}
	if($('#subscribe-modal, #logged-modal').length > 0) {
		$dialogShown = false;

		setInterval(function(){
			console.log("Let's subscribe");
			var date = new Date();
			var last_date_str = localStorage.getItem('lastSignUpIntent');
			if (last_date_str != null){

				var last_date = parseInt(last_date_str);
				//if(!$dialogShown) {
				if (date.getTime() > last_date + (5*60*1000)){

					localStorage.setItem('lastSignUpIntent', date.getTime()+"");
	...
	$(window).keydown(function(event){
		if(event.keyCode == 13) {
			var focus = $("#search_term").is(":focus");
			if (focus == false){
	...
	setTimeout(function(){
		$(".fixed-alert").hide(500);
	}, 5000);

	$(".hide-toggler").change(function(){
		hideToggler(this);
	});
	...
	function hideToggler(element){
		var id = $(element).data("toggle");
		var inverse = $(element).data("inverse");
	...
	$(".currency").keydown(function(){
		...
		var v = parseInt($(this).val()).formatMoney(0, '.', ',');
	...
	function renderTimelines () {
		if ($("#twitter-timeline-container").children().length == 0){

			if (typeof twttr !== 'undefined' && twttr !== null){
				twttr.widgets.createTimeline(
					{
						sourceType: 'url',
						url: 'https://twitter.com/AgroNetoOficial',
						lang: "es"
					},
